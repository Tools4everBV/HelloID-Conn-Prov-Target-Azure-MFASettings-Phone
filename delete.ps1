...
try {
    ...
    Write-Verbose "Querying Azure AD user with id $($aRef)"
    ...
    if ($auditErrorMessage -Like "*Resource '$($aRef)' does not exist*") {
        if (-Not($dryRun -eq $True)) {
            $auditLogs.Add([PSCustomObject]@{
                    ...
                    Message = "No Azure AD user found with id $($aRef). Possibly already deleted, skipping action."
                    IsError = $false
                })
        }
        else {
            Write-Warning "DryRun: No Azure AD user found with id $($aRef). Possibly already deleted, skipping action."
        }        
    }
    else {
        $success = $false  
